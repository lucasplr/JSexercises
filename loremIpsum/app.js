...
form.addEventListener("submit", function(e){
    e.preventDefault() //the form in default sent the data to a server, and in this case, this is needed
    const value = parseInt(amount.value) //parseInt is needed because the value getted is in string.
    console.log(value)
    //empty
    // -1
    // > 9
    ...
    if (value > 9 || value <= 0 || isNaN(value)){
        let randomT = text[Math.floor(Math.random() * text.length)]
        result.innerHTML = `<p class="result">${randomT}</p>`
    }else{
